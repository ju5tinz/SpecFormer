...
CMS2Spectrum::CMS2Spectrum(long n, float *mass, float *intensity, std::string sequence, int charge, float CollisionEnergy, float precursor)
{
	using namespace std;
...
/*	if (MetOx0)
	...
	for (j = 30; j < 72; j++)
	{
		...
		CheckMass(FragName, m_Mass0 - j * 1.0005, 0.5f, m_charge, 0.05f);	//b ion
	}
	...
	for (i = 0; i < length-1; i++)
	{
		...
		likelihood = CleavageLikelihood(i);	//b/y likelihood close to 1
		bMass += ResidueMass[i];
		sprintf(FragName, "b%d", i + 1);
		CheckMass(FragName, bMass, likelihood, m_charge, 0.002f);	//b ion
		
		sprintf(FragName, "y%d", length - i - 1);
		CheckMass(FragName, m_Mass0 - bMass, likelihood, m_charge, 0.002f);	//y ion

		if (NH3loss)
		{
			sprintf(FragName, "b%d-17", i+1);
			CheckMass(FragName, bMass - NH3, likelihood*0.95f, m_charge, 0.002f);	//b ion
		}
		if (NH3loss0 - NH3loss)
		{
			sprintf(FragName, "y%d-17", length - i - 1);
			CheckMass(FragName, m_Mass0 - bMass - NH3, likelihood * 0.95f, m_charge, 0.002f);	//y ion
		}

		if (H2Oloss)
		{
			sprintf(FragName, "b%d-18", i + 1);
			CheckMass(FragName, bMass - H2O, likelihood * 0.95f, m_charge, 0.002f);	//b ion
		}
		if (H2Oloss0 - H2Oloss)
		{
			sprintf(FragName, "y%d-18", length - i - 1);
			CheckMass(FragName, m_Mass0 - bMass - H2O, likelihood * 0.95f, m_charge, 0.002f);	//y ion
		}

		// a ion
		sprintf(FragName, "b%d-28", i + 1);
		CheckMass(FragName, bMass - CO, likelihood * 0.95f, m_charge, 0.002f);	//a ion

/*		if (MetOx)
		{
			sprintf(FragName, "b%d-64", i + 1);
			CheckMass(FragName, bMass - 63.9983, likelihood, m_charge, 0.002f);	//b ion
		}
		if (MetOx0 - MetOx)
		{
			sprintf(FragName, "y%d-64", length - i - 1);
			CheckMass(FragName, m_Mass0 - bMass - 63.9983, likelihood, m_charge, 0.002f);	//y ion
		}*/

		if (NumU)
		{
			sprintf(FragName, "b%d-91", i + 1);
			CheckMass(FragName, bMass - 91.009184, likelihood*0.9f, m_charge, 0.002f);	//b ion
		}
		if (NumU0 - NumU)
		{
			sprintf(FragName, "y%d-91", length - i - 1);
			CheckMass(FragName, m_Mass0 - bMass - 91.009184, likelihood*0.9f, m_charge, 0.002f);	//y ion
		}

		if (NumJ)
		{
			sprintf(FragName, "b%d-92", i + 1);
			CheckMass(FragName, bMass - 91.993200, likelihood * 0.9f, m_charge, 0.002f);	//b ion
		}
		if (NumJ0 - NumJ)
		{
			sprintf(FragName, "y%d-92", length - i - 1);
			CheckMass(FragName, m_Mass0 - bMass - 91.993200, likelihood * 0.9f, m_charge, 0.002f);	//y ion
		}

		if (PhosY)
		{
			sprintf(FragName, "b%d-80", i + 1);
			CheckMass(FragName, bMass - 79.966330, likelihood, m_charge, 0.002f);	//b ion
		}
		if (PhosY0 - PhosY)
		{
			sprintf(FragName, "y%d-80", length - i - 1);
			CheckMass(FragName, m_Mass0 - bMass - 79.966330, likelihood, m_charge, 0.002f);	//y ion
		}

		if (PhosST + PhosY)
		{
			sprintf(FragName, "b%d-98", i + 1);
			CheckMass(FragName, bMass - 97.976895, likelihood, m_charge, 0.002f);	//b ion
		}
		if (PhosST0 + PhosY0 - PhosST - PhosY)
		{
			sprintf(FragName, "y%d-98", length - i - 1);
			CheckMass(FragName, m_Mass0 - bMass - 97.976895, likelihood, m_charge, 0.002f);	//y ion
		}

		//additions to form c, x ions
		sprintf(FragName, "b%d+17", i + 1);
		CheckMass(FragName, bMass + NH3, likelihood * 0.8f, m_charge, 0.002f);	//c ion

		sprintf(FragName, "b%d+18", i + 1);
		CheckMass(FragName, bMass + H2O, likelihood * 0.8f, m_charge, 0.002f);	//bn-1 + H2O C-term rearrangement

		sprintf(FragName, "y%d+28", length - i - 1);
		CheckMass(FragName, m_Mass0 - bMass + CO, likelihood * 0.5f, m_charge, 0.002f);	//x ion

		//other neutral losses
		for (j = 30; j < 72; j++)
		{
			sprintf(FragName, "b%d-%d", i + 1, j);
			CheckMass(FragName, bMass - j*1.0005, likelihood * 0.5f, m_charge, 0.05f);	//b ion

			sprintf(FragName, "y%d-%d", length - i - 1, j);
			CheckMass(FragName, m_Mass0 - bMass - j*1.0005, likelihood * 0.5f, m_charge, 0.05f);	//y ion
		}
	}
	...
	for (i = 0; i < length - 3; i++)	//first cut (on the right side of i)
	{
		...
		for (j = i + 1; j < length - 1; j++)	//second cut
		{
			...
			if (j > i + 1)	//at least two residues
			{
				...
				CheckMass(FragName, fragmass, likelihood, m_charge, 0.002f);
				sprintf(FragName, "[%d_%d]-28", i + 2, j + 1);
				CheckMass(FragName, fragmass-CO, likelihood*0.9f, m_charge, 0.002f);	//internal immonium
			}
		}
	}
	...
	for (i = 0; i < length; i++)
	{
		...
		if (!skip)
		{
			sprintf(FragName, "%c+28", m_sequence[i]);	//amino acid acyl
...
int CMS2Spectrum::CheckMass(char FragName[], double FragMass, float likelihood, int MaxCharge, float MinTolerance)
{
	...
	int n = IsotopeSimulationFromMass(FragMass, (int)(7.0 + FragMass/1000.0), a, 0.05f, &top);	//top is the index of top of a[]
	float max = a[top];
	double mz, topmz;
	float currentmz;
	...
	for (z = z1; z <= z2; z++)
	{
		...
		while (j < n && i < m_points)
		{
			...
			while (i < m_points && m_mass[i] < currentmz - accuracy)
				i++;
			while (i < m_points && m_mass[i] < currentmz + accuracy)
			{
		...
		while (j >= 0 && i >= 0)
		{
			...
			while (i >= 0 && m_mass[i] > currentmz + accuracy)
				i--;
			while (i >= 0 && m_mass[i] > currentmz - accuracy)
			{
				if (a2[j] < m_intensity[i])
...
void CMS2Spectrum::ExportAnnotations(char filename[], CFeatureList *FeatureList)
{
	using namespace std;
	...
	for (k = 0; k < FeatureList->m_NumFeatures; k++)
		FeatureIntensity[k] = 0.0f;
	...
	for (j = 0; j < m_points; j++)
	{
		if (m_label[j].length() > 0)
		{
			...
			while ((b = (int)m_label[j].find('\n', iStart)) > 0)
			{
				...
				for (k = 0; k < FeatureList->m_NumFeatures; k++)
				{
					if (FeatureList->m_Features[k] == OneLabel)
					{
						FeatureIntensity[k] = m_LabelIntensity[j];
						success = 1;
						break;
					}
				}
	...
	for (k = 0; k < FeatureList->m_NumFeatures; k++)
	{
		if (FeatureIntensity[k] > 0.5f)
		{
			file << k << ',' << FeatureList->m_Features[k] << ',' << (int)(FeatureIntensity[k] + 0.5f) << '\n';
		}
	}
