...
class DataLoaderIter(object):
    ...
    def inputs_labels_from_batch(self, batch_data):
        ...
        inputs, labels, charges, NCEs = batch_data

        return inputs, labels, charges, NCEs
...
class TrainDataLoaderIter(DataLoaderIter):
    ...
    def __next__(self):
        try:
            batch = next(self._iterator)
            inputs, labels, charges, NCEs = self.inputs_labels_from_batch(batch)
...
class LRFinder(object):
    ...
    def _train_batch(self, train_iter, accumulation_steps, non_blocking_transfer=True):
        ...
        for i in range(accumulation_steps):
            inputs, labels, charges, NCEs = next(train_iter)
            inputs, labels, charges, NCEs = self._move_to_device(
                inputs, labels, charges, NCEs, non_blocking=non_blocking_transfer
            )
            ...
            if self.amp_backend == "torch":
                ...
                outputs = self.model(inputs, charges, NCEs)
                loss = self.criterion(outputs, labels)
    ...
    def _move_to_device(self, inputs, labels, charges, NCEs, non_blocking=True):
        ...
        charges = move(charges, self.device, non_blocking=non_blocking)
        return inputs, labels, charges, NCEs
    ...
    def _validate(self, val_iter, non_blocking_transfer=True):
        ...
        self.model.eval()
        with torch.no_grad():
            for inputs, labels, charges, NCEs in val_iter:
